...
TIMEFRAME = os.getenv('TIMEFRAME', '1h')  # 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
...
def load_state():
    ...
    try:
        ...
        if state.get('symbol') != SYMBOL:
            logger.warning(f"State file is for a different symbol ({state.get('symbol')}, current: {SYMBOL}). Not loading state.")
            return
...
def log_position_state():
    ...
    if position_active:
        ...
        try:
            ticker = Client(API_KEY, API_SECRET, testnet=USE_TESTNET).get_ticker(symbol=SYMBOL)
            ...
            if position_side == 'BUY':
                ...
                pnl_percent = (Decimal('1') - current_price / entry_price) * Decimal('100')
                
            logger.info(f"  Current Price: {format_price(current_price)}")
            logger.info(f"  Unrealized P&L: {format_price(pnl)} {QUOTE_ASSET} ({pnl_percent:.2f}%)")
        except Exception as e:
...
def update_position_state(order_result, side, atr_value=None):
    ...
    if side == 'BUY' and not position_active:
        ...
        if atr_value is not None:
            ...
            take_profit_price = entry_price + (stop_distance * Decimal('2'))  # 2:1 reward-to-risk
            
            logger.info(f"Position opened: LONG {position_quantity} {BASE_ASSET} at {format_price(entry_price)}")
            logger.info(f"Stop loss set at {format_price(stop_loss_price)} (ATR: {entry_atr})")
        ...
        if atr_value is not None:
            ...
            logger.info(f"Position opened: SHORT {position_quantity} {BASE_ASSET} at {format_price(entry_price)}")
            logger.info(f"Stop loss set at {format_price(stop_loss_price)} (ATR: {entry_atr})")
            logger.info(f"Take profit set at {format_price(take_profit_price)}")
        else:
            logger.warning("ATR value not provided. Stop loss and take profit not set.")
        ...
        logger.info(f"Entry: {format_price(entry_price)}, Exit: {format_price(exit_price)}")
        ...
        logger.warning(f"Unexpected order scenario: side={side}, position_active={position_active}, position_side={position_side}")
...
def setup_logging():
    ...
    # Create file handler
    file_handler = logging.FileHandler(log_dir / f'turtle_bot_{time.strftime("%Y%m%d")}.log')
...
def initialize_binance_client():
    if not API_KEY or not API_SECRET:
        logger.error("API Key or Secret Key not found. Please set them in the environment variables.")
    ...
    try:
        ...
        logger.info(f"Account status: {account_info['accountType']}, canTrade: {account_info['canTrade']}")
...
def calculate_indicators(df, dc_enter, dc_exit, atr_len, atr_smooth):
    ...
    try:
        ...
        result['dc_upper_entry'] = result['high'].rolling(window=dc_enter).max().shift(1)
        result['dc_lower_entry'] = result['low'].rolling(window=dc_enter).min().shift(1)
        result['dc_middle_entry'] = (result['dc_upper_entry'] + result['dc_lower_entry']) / 2
        ...
        result['dc_middle_exit'] = (result['dc_upper_exit'] + result['dc_lower_exit']) / 2
        ...
        logger.info(f"Calculated indicators with parameters: dc_enter={dc_enter}, dc_exit={dc_exit}, atr_len={atr_len}, atr_smooth={atr_smooth}")
...
def fetch_data(client, symbol, interval, lookback):
    ...
    try:
        ...
        if len(df) < lookback:
            logger.warning(f"Insufficient data: got {len(df)} candles, wanted {lookback}")
            return df
...
def round_step_size(quantity, step_size):
    ...
    return float(Decimal(int(quantity * Decimal(10**precision)) / Decimal(10**precision)))
...
def calculate_trade_size(balance, risk_percent, atr_value, current_price, min_qty, step_size, min_notional, price_precision):
    """
    ...
    try:
        ...
        logger.info(f"Risk amount: {risk_amount} {QUOTE_ASSET} ({risk_percent*100}% of {balance})")
        ...
        logger.info(f"Stop distance: {stop_distance} ({STOP_LOSS_ATR_MULTIPLE} * ATR {atr_value})")
        ...
        logger.info(f"Position size adjusted for step size: {adjusted_position_size} {BASE_ASSET}")
        
        # 5. Check against minimum quantity requirement
        if adjusted_position_size < min_qty:
            logger.warning(f"Calculated position size {adjusted_position_size} is below minimum quantity {min_qty}")
            
            # Try to use minimum quantity instead
            adjusted_position_size = min_qty
            logger.info(f"Adjusted to minimum quantity: {adjusted_position_size} {BASE_ASSET}")
        
        # 6. Check against minimum notional value
        notional_value = adjusted_position_size * current_price
        
        if notional_value < min_notional:
            logger.warning(f"Notional value {notional_value} is below minimum {min_notional}")
            ...
            adjusted_position_size = Decimal(round_step_size(required_position_size, step_size))
            ...
            if adjusted_position_size % step_size != Decimal('0'):
                ...
                steps = (adjusted_position_size / step_size).quantize(Decimal('1'), rounding='ROUND_UP')
                adjusted_position_size = steps * step_size
            
            logger.info(f"Position size adjusted for min notional: {adjusted_position_size} {BASE_ASSET}")
            notional_value = adjusted_position_size * current_price
        ...
        if required_balance > balance:
            logger.warning(f"Required balance {required_balance} exceeds available balance {balance}")
            
            # Try to adjust position size to available balance
            max_affordable_size = (balance * Decimal('0.99')) / current_price  # 99% of balance for fees
            adjusted_position_size = Decimal(round_step_size(max_affordable_size, step_size))
            
            # Final check against minimum requirements
            if adjusted_position_size < min_qty:
                logger.error(f"Cannot meet minimum quantity requirement with available balance")
                return Decimal('0')
            
            notional_value = adjusted_position_size * current_price
            if notional_value < min_notional:
                logger.error(f"Cannot meet minimum notional requirement with available balance")
                return Decimal('0')
            
            logger.info(f"Position size adjusted for available balance: {adjusted_position_size} {BASE_ASSET}")
        
        # 8. Final check for all requirements
        if (adjusted_position_size >= min_qty and 
            adjusted_position_size * current_price >= min_notional and
            adjusted_position_size * current_price <= balance):
            ...
            logger.info(f"Risk per trade: {risk_amount} {QUOTE_ASSET} ({risk_percent*100}%)")
            
            return final_size
        else:
            logger.error("Failed to calculate valid position size meeting all requirements")
            return Decimal('0')
        ...
        if position_active:
            ...
            if position_side == 'BUY':  # We're in a LONG position
                ...
                if current_price <= stop_loss_price:
                    logger.info(f"STOP LOSS TRIGGERED: Current price {format_price(current_price)} <= Stop loss {format_price(stop_loss_price)}")
                    ...
                    if success:
                        ...
                        logger.error(f"Failed to execute stop loss order: {order_result}")
                
                # Check Donchian channel exit (price breaks below lower band)
                elif latest_row['close'] < latest_row['dc_lower_exit']:
                    logger.info(f"EXIT SIGNAL: Close price {latest_row['close']} broke below Donchian lower band {latest_row['dc_lower_exit']}")
                    ...
                    if success:
                        logger.info("Successfully closed LONG position with Donchian exit signal")
                    else:
                        logger.error(f"Failed to execute Donchian exit order: {order_result}")
                
                else:
                    logger.info("No exit conditions met, maintaining LONG position")
                    
            elif position_side == 'SELL':  # We're in a SHORT position - for future implementation
            ...
            if latest_row['close'] > latest_row['dc_upper_entry']:
                logger.info(f"ENTRY SIGNAL: Close price {latest_row['close']} broke above Donchian upper band {latest_row['dc_upper_entry']}")
                ...
                if position_size > Decimal('0'):
                    logger.info(f"Calculated position size: {position_size} {BASE_ASSET}")
                    ...
                    logger.warning("Entry signal detected but position size calculation returned zero. No order placed.")
            else:
...
def execute_order(client, symbol, side, quantity, price_precision, quantity_precision, simulate=False, atr_value=None):
    """
    ...
    try:
        ...
        # Log order details
        logger.info(f"Preparing to execute {side} order for {formatted_quantity} {symbol}")
        ...
        if simulate or USE_TESTNET:
            ...
            logger.info(f"Executing {execution_type} {side} MARKET order for {formatted_quantity} {symbol} at ~{formatted_price}")
            ...
            commission_asset = symbol[-4:] if len(symbol) >= 4 else 'USDT'  # Use quote asset for commission
            commission_amount = (Decimal(formatted_quantity) * execution_price * commission_rate)
            ...
            order_result = {
                ...
                'cummulativeQuoteQty': str(Decimal(formatted_quantity) * execution_price),
                'status': 'FILLED',
                'timeInForce': 'GTC',
                'type': 'MARKET',
                'side': side,
            ...
            logger.info(f"Executing REAL {side} MARKET order for {formatted_quantity} {symbol}")
        ...
        if order_result['status'] == 'FILLED':
            ...
            logger.info(f"  Average Fill Price: {format_price(avg_price, price_precision)}")
            ...
            logger.warning(f"Order {order_result['orderId']} created but status is {order_result['status']}")
...
def get_sleep_time(timeframe):
    ...
    if timeframe == '1m':
        ...
        seconds_to_next_close = hours_remaining * 3600 + (60 - current_time.tm_min) * 60 - current_time.tm_sec
        ...
        seconds_to_midnight = (24 - current_time.tm_hour - 1) * 3600 + (60 - current_time.tm_min) * 60 - current_time.tm_sec
        seconds_to_next_close = seconds_to_midnight
        
    else:
        # Default: check every 5 minutes
        logger.warning(f"Unrecognized timeframe {timeframe}, defaulting to 5-minute checks")
...
def run_bot():
    ...
    logger.info(f"Strategy Parameters: DC_Enter={DC_LENGTH_ENTER}, DC_Exit={DC_LENGTH_EXIT}, ATR_Length={ATR_LENGTH}")
    logger.info(f"Risk Management: Risk_Per_Trade={RISK_PER_TRADE}, SL_ATR_Multiple={STOP_LOSS_ATR_MULTIPLE}")
            ...
            if entry_atr == Decimal('0') or stop_loss_price == Decimal('0'):
                ...
                if df is not None and not df.empty:
                    ...
                    if df_with_indicators is not None and not df_with_indicators.empty:
                        ...
                        if position_side == 'BUY':
                            stop_loss_price = entry_price - stop_distance
                            take_profit_price = entry_price + (stop_distance * Decimal('2'))
                        else:
                            stop_loss_price = entry_price + stop_distance
                            take_profit_price = entry_price - (stop_distance * Decimal('2'))
                        
                        logger.info(f"Updated stop loss to {format_price(stop_loss_price)}")
                        logger.info(f"Updated take profit to {format_price(take_profit_price)}")
                        save_state()
        ...
        while True:
            try:
                ...
                if df is None or df.empty:
                    logger.error("Unable to fetch historical data. Skipping this iteration.")
                ...
                if df_with_indicators is None or df_with_indicators.empty:
                    logger.error("Failed to calculate indicators. Skipping this iteration.")
                    time.sleep(60)  # Wait 60 seconds before retrying
                    continue
                ...
                logger.info(f"Next check at: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(next_check_time))}")
                ...
                if hasattr(e, 'code'):
                    if e.code == -1021:  # RECONNECT
                        logger.error("Timestamp for this request was outside the recvWindow. Server time might be different. Syncing...")
                        client.ping()  # Ping to sync time
                    elif e.code == -1003:  # TOO_MANY_REQUESTS
                        logger.error("Rate limit exceeded. Waiting longer before next request.")
                        time.sleep(300)  # Wait 5 minutes
                    else:
                        logger.error(f"Unhandled API error code: {e.code}. Waiting before retry.")
